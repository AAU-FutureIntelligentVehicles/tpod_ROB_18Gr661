  ...
  std_msgs::Float64 brake_position;
	...
	while(ros::ok()){
		...
		if (turningAngle != 1000){													//Loop for driving when no obstacle is in front of us
			...
			//safety stuff so we dont try to turn the steering wheel more than the car is physically capable of
			if (beta >= 22000){
				beta = 22000;
			}
			else if (beta <= -22000){
		...
		else if (turningAngle == 1000){											//This is the loop for braking, when no usable path points are received
