...
def main():
    ...
    while i < 5:
        ...
        if zed.grab(runtime_parameters) == tp.PyERROR_CODE.PySUCCESS:
            ...
            #print(confidence.get_data())
            print(np.max(confidence.get_data()))
            a=np.dstack((image.get_data()[...,0]/256, np.nan_to_num(point_cloud.get_data()[...,1]/7000), confidence.get_data()/100))
            plt.imshow(a)
            plt.show()
            ...
            for window in windows:
                ...
                hog_f = HOG_lib.single_img_features(result)
                #for channel in range(result.shape[2]):

                #    hog_f.extend(HOG_lib.get_hog_features(result[:, :, channel], orient, pix_per_cell, cell_per_block, vis=False, feature_vec=True))
    ...
    scaled_X, y = HOG_lib.prepare_images_for_processing(car_folders, notcar_folders, "png")
    ...
    for c, image, i in zip(classes, images, range(len(images))):
        print("sorted_data/{0}/{1}.jpg".format(int(c), i))
        cv2.imwrite("sorted_data/{0}/{1}.jpg".format(int(c), i), image)
